...
function ParentSignup(props) {
  ...
  const handleSubmit = async (e) => {
      ...
      e.preventDefault();
      const response = await fetch("http://localhost:5000/parentpost", {
          method: 'POST',
          headers: {
      ...
      console.log(json);
      if (json.success){
          localStorage.setItem('parentEmail', credentials.email); 
  ...
  return (
    ...
    <>
    <Navbar title="Charity Management System - Parent Sign Up" />
      <style>{styles}</style>
      <div 
